...
function TabelaAgrupada({ titulo, subtitulo, linhas, rotuloChave }: { titulo: string; subtitulo?: string; linhas: LinhaAgrupada[]; rotuloChave: string }) {
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
      ...
      {subtitulo && <p className="text-slate-600 text-xs mb-4">{subtitulo}</p>}
      {linhas.length === 0 ? (
        <p className="text-slate-600 text-sm">Sem dados suficientes ainda.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {linhas.map(l => (
                <tr key={l.chave}>
                  <td className="py-2.5 pr-4 text-slate-300">{l.chave}</td>
                  ...
                  <td className="py-2.5 pl-4 text-slate-400 text-right">{fmtPct(l.cvrMedio, 1)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export function DashboardTestesView({ testes }: Props) {
  ...
  if (testes.length === 0) {
    return (
      <div className="bg-slate-900 border border-dashed border-slate-800 rounded-xl p-14 flex flex-col items-center justify-center text-center gap-3">
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {testesAtivos.length === 0 ? (
          <p className="text-slate-600 text-sm">Nenhum teste ativo agora com esses filtros.</p>
        ) : (
          <div className="overflow-x-auto">
                ...
                {testesAtivos.map(t => {
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-slate-900/60 transition-colors">
                      ...
                      <td className="py-2.5 px-4 text-slate-400">{t.funis?.nome ?? '—'}</td>
                      <td className="py-2.5 px-4 text-slate-400">{t.campanhas?.codigo ?? '—'}</td>
                      <td className="py-2.5 px-4 text-slate-400">{t.segmento ?? '—'}</td>
                      <td className="py-2.5 px-4 text-slate-400">{t.elemento_testado ?? '—'}</td>
                      <td className="py-2.5 px-4 text-slate-400">{resumoLayout(t.variantes_teste ?? []) ?? '—'}</td>
                      <td className={`py-2.5 px-4 text-right ${dias !== null && dias > 14 ? 'text-amber-400' : 'text-slate-400'}`}>
                        {dias !== null ? `${dias}d` : '—'}
                      </td>
                      <td className="py-2.5 pl-4 text-right">
                        {url && (
                          <a href={url} target="_blank" rel="noopener noreferrer" className="text-slate-600 hover:text-indigo-400 transition-colors inline-flex" title="Acessar URL de ativação">
